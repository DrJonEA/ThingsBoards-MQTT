...
MQTTStatus_t MQTTAgent::init(){
	...
	//Agent Command Context for callbacks
	xAgentCommandContext.object = (void *) this;
		 ...
		 switch(xConnState){
			 ...
			 case TCPConned: {
				 ...
				 if (status == MQTTSuccess){
					 setConnState(Online); // Start Command Loop
...
void MQTTAgent::connectCmdCallback( MQTTAgentCommandContext_t * pCmdCallbackContext,
	...
	MQTTAgent * agent = (MQTTAgent *)pCmdCallbackContext->object;
	agent->connectCmdHandler(pCmdCallbackContext, pReturnInfo );
}

void MQTTAgent::connectCmdHandler( MQTTAgentCommandContext_t * pCmdCallbackContext,
            MQTTAgentReturnInfo_t * pReturnInfo ){
	...
	//Subscriber if we have a router
	if (pRouter != NULL){
...
MQTTStatus_t  MQTTAgent::MQTTconn(){
	...
	//Setup Agent Connect Arguments
	xAgentConnectArgs.pConnectInfo = &xConnectInfo;
	xAgentConnectArgs.pWillInfo = &xWillInfo;
	xAgentConnectArgs.timeoutMs = MQTT_CONNECT_TIMEOUT;
	xAgentConnectArgs.sessionPresent = false;

	xCommandInfo.cmdCompleteCallback = MQTTAgent::connectCmdCallback;
	xCommandInfo.pCmdCompleteCallbackContext = &xAgentCommandContext;
	xCommandInfo.blockTimeMs = 0;

	//Do Connection
	xResult = MQTTAgent_Connect(
...
bool MQTTAgent::pubToTopic(const char * topic, const void * payload,
	...
	if (xConnState != Online){
		LogWarn(("pubToTopic when not online"));
		return false;
	}
